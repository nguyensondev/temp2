 ...
 * and modified slightly to account for our Op3n higher order components,
...
describe("i18n", () => {
  test("There are no missing keys", (done) => {
    ...
    // grep "Tx=\"\S*\"\|tx=\"\S*\"\|translate(\"\S*\"" -ohr './src' | grep -o "\".*\""
    const command = `grep "Tx=\\"\\S*\\"\\|tx=\\"\\S*\\"\\|translate(\\"\\S*\\"" -ohr './src' | grep -o "\\".*\\""`
